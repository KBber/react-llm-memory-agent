...
def run(
    ...
    多轮对话参数：
      session_id  会话标识，传入后开启短期记忆
      memory      已存在的 ShortTermMemory 实例（如调用方自行管理）
    """
    ...
    for step in range(1, max_steps + 1):
        ...
        # for-else：达到 max_steps 自然结束（未走 final 分支）
        yield {
            "step":   max_steps + 1,
            "type":   "max_steps",
